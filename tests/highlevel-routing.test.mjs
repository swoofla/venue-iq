...
async function run(name, venue, {missing=false, mismatch=false, fail=false, failSms=false, dates={}}={}) {
 ...
 const context={Response,Date,Intl,console:{log(){},warn(){},error(){}},createClientFromRequest:()=>({asServiceRole:{entities},entities}),Deno:{env:{get:k=>env[k]},serve:fn=>handler=fn},fetch:async(url,opts)=>{calls.push({url,...opts});if(failSms && url.endsWith('/conversations/messages'))return Response.json({message:'The token is not authorized for this scope.'},{status:401});if(fail)return Response.json({error:'failure'},{status:400});if(url.includes('free-slots'))return Response.json({'2026-09-25':{slots:['2026-09-25T15:00:00Z']}});if(url.includes('/calendars/') && !url.includes('/events/'))return Response.json({calendar:{locationId:venue===conrad?'conrad-location':'sugar-location',isActive:true}});if(url.includes('contacts/upsert'))return Response.json({contact:{id:'contact'}});return Response.json({id:'id',messageId:'message'});}};
...
const smsFailure=await run('createHighLevelLeadAndNotify',conrad,{failSms:true});
assert.equal(smsFailure.payload.success,false);
